...
![](https://gitlab.com/nvidia-networking/systems-engineering/poc-support/kubernetes-on-baremetal-network/-/raw/main/gitops.png)
...
![](https://gitlab.com/nvidia-networking/systems-engineering/poc-support/kubernetes-on-baremetal-network/-/raw/main/diagram.png)
...
| leaf1, leaf2 | CL 4.4 | cumulus | CumulusLinux! | 
| spine | Sonic 202012 | admin | YourPaSsWoRd | 
| kubernetes | v1.16.2 | n/a | n/a |
...
This walkthrough is designed for the "Kubernetes on Baremetal Network" demo hosted in [Air Marketplace](https://air.nvidia.com/marketplace).
...
Connect to the `oob-mgmt-server` (cumulus/CumulusLinux!) and ssh into the `netq-ts` node, which is running the Kubernetes control plane, and run the following commands:
...
### 4. Create a kustomization resource for dc network
...
dc1   True    Applied revision: main/db600c7531a205a6de8110932d1f5e83b00637e9   15s
```

>  **[NOTE]** For more details about how GitOps is configured to work under the hood see the [`./gitops.md`](https://gitlab.com/nvidia-networking/systems-engineering/poc-support/kubernetes-on-baremetal-network/-/blob/main/gitops.md)


### 5. Observe applied changes
...
As the result of these changes, we will be able to see a new set of IPs on the loopback interfaces

```
cumulus@oob-mgmt-server:~$ ssh leaf1 -C 'ip -br addr show dev lo'; ssh leaf2 -C 'ip -br addr show dev lo'; ssh admin@spine -C 'ip -br addr show dev Loopback0'
lo               UNKNOWN        127.0.0.1/8 192.0.2.1/32 198.51.100.1/32 ::1/128
lo               UNKNOWN        127.0.0.1/8 192.0.2.2/32 198.51.100.2/32 ::1/128
Loopback0        UNKNOWN        192.0.2.100/32 198.51.100.100/32 fe80::648d:d8ff:fe21:3046/64
```

And we'd able to ping those new loopback IPs:
...
231dd30 Change #738 - Adding extra loopback interfaces
638514f Thu Sep 23 14:23:38 BST 2021
```
...
A few seconds later the change is reverted and all of the new loopback IPs are gone:

```
cumulus@oob-mgmt-server:~$ ssh leaf1 -C 'ip -br addr show dev lo'; ssh leaf2 -C 'ip -br addr show dev lo'; ssh admin@spine -C 'ip -br addr show dev Loopback0'
lo               UNKNOWN        127.0.0.1/8 192.0.2.1/32 ::1/128
lo               UNKNOWN        127.0.0.1/8 192.0.2.2/32 ::1/128
Loopback0        UNKNOWN        192.0.2.100/32 fe80::f6:d4ff:fecb:356f/64
```
<!-- AIR:tour -->